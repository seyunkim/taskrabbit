...
Tasker who is hired?} Answered with the assumption the question wants
the average of the numerical position per category
...
\end{Verbatim}
    ...
    9.) \textbf{For each category, what is the average hourly rate and
...
\end{Verbatim}
    ...
    10.) \textbf{Based on the previous, how would you approach the question
of: How can we use market data to suggest hourly rates to Taskers that
would maximize their opportunity to be hired? Please describe in detail,
with code and formulas that support your model.}

    2 methods that come to mind * 1st would be to gather data on the range
of values to calculate standard deviation and the average rate at which
taskers get hired and give task-seekers a market range in which they are
most likely to get hired. * 2nd would be to create a machine learning
prediction model (decision tree-based or something as simple as
multivariate logistic regression) that can use past data to predict a
binary classification of whether a tasker will get hired or not hired
based on tasker's input of hourly rate and category they are working
for.
